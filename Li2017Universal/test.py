...
def styleTransfer(content, sFs, wct_mode):
    ...
    if wct_mode == 'gpu':
        ...
        csF5 = wct.transform(cF5, sF5, args.alpha, index=1, wct_mode=wct_mode)
    else:
        cF5 = wct.e5(content).data.cpu().squeeze(0)
        csF5 = wct.transform(cF5, sF5, args.alpha, index=1, wct_mode=wct_mode).to(device)
    Im5 = wct.d5(csF5)
    ...
    if wct_mode == 'gpu':
        ...
        csF4 = wct.transform(cF4, sF4, args.alpha, index=2, wct_mode=wct_mode)
    else:
        cF4 = wct.e4(Im5).data.cpu().squeeze(0)
        csF4 = wct.transform(cF4, sF4, args.alpha, index=2, wct_mode=wct_mode).to(device)
    Im4 = wct.d4(csF4)
    ...
    if wct_mode == 'gpu':
        ...
        csF3 = wct.transform(cF3, sF3, args.alpha, index=3, wct_mode=wct_mode)
    else:
        cF3 = wct.e3(Im4).data.cpu().squeeze(0)
        csF3 = wct.transform(cF3, sF3, args.alpha, index=3, wct_mode=wct_mode).to(device)
    Im3 = wct.d3(csF3)
    ...
    if wct_mode == 'gpu':
        ...
        csF2 = wct.transform(cF2, sF2, args.alpha, index=4, wct_mode=wct_mode)
    else:
        cF2 = wct.e2(Im3).data.cpu().squeeze(0)
        csF2 = wct.transform(cF2, sF2, args.alpha, index=4, wct_mode=wct_mode).to(device)
    Im2 = wct.d2(csF2)
    ...
    if wct_mode == 'gpu':
        ...
        csF1 = wct.transform(cF1, sF1, args.alpha, index=5, wct_mode=wct_mode)
    else:
        cF1 = wct.e1(Im2).data.cpu().squeeze(0)
        csF1 = wct.transform(cF1, sF1, args.alpha, index=5, wct_mode=wct_mode).to(device)
    Im1 = wct.d1(csF1)
    ...
    for i in range(repeat):
        ...
        if args.URST:
            ...
            with torch.no_grad():
                ...
                # style_transfer_high_resolution(
                #     patches, sFs, padding=PADDING, collection=True, wct_mode="gpu",
