...
def tensor_save_bgrimage(tensor, filename, cuda=False):
    tensor = tensor_bgr2rgb(tensor)
    tensor_save_rgbimage(tensor, filename, cuda)

def tensor_bgr2rgb(tensor):
    (b, g, r) = torch.chunk(tensor, 3)
    tensor = torch.cat((r, g, b))
    return tensor

def tensor_rgb2bgr(tensor):
    (r, g, b) = torch.chunk(tensor, 3)
    tensor = torch.cat((b, g, r))
    return tensor
